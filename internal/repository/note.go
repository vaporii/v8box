...
type NoteRepository interface {
	CreateNote(id string, userID string, note *dto.CreateNoteRequest) (*models.Note, error)
...
func (r *noteRepository) CreateNote(id string, userID string, note *dto.CreateNoteRequest) (*models.Note, error) {
	...
	`, id, userID, note.Title, note.Content).Scan(&retNote.ID, &retNote.UserID, &retNote.Title, &retNote.Content, &retNote.CreatedAt, &retNote.UpdatedAt)
	if err != nil {
		return nil, err
	}
